...
ASM_SOUND_SOURCES=$(wildcard $(SOUND_DIR)/asm/*.asm)
ASM_STRINGED_SOURCES=$(wildcard $(STRINGED_DIR)/asm/*.asm)
ASM_UI_MP_SOURCES=$(wildcard $(UI_MP_DIR)/asm/*.asm)
...
ASM_SOUND_OBJ=$(patsubst $(SOUND_DIR)/asm/%.asm,$(OBJ_DIR)/%.o,$(ASM_SOUND_SOURCES))
ASM_STRINGED_OBJ=$(patsubst $(STRINGED_DIR)/asm/%.asm,$(OBJ_DIR)/%.o,$(ASM_STRINGED_SOURCES))
ASM_UI_MP_OBJ=$(patsubst $(UI_MP_DIR)/asm/%.asm,$(OBJ_DIR)/%.o,$(ASM_UI_MP_SOURCES))
...
cod2rev: mkdir $(TARGET)
    $(TARGET): \
	$(ASM_BOTLIB_OBJ) $(ASM_CLIENTSCR_OBJ) $(ASM_GAME_OBJ) $(ASM_QCOMMON_OBJ) $(ASM_SERVER_OBJ) $(ASM_SOUND_OBJ) $(ASM_STRINGED_OBJ) \
	$(ASM_UI_MP_OBJ) $(ASM_UNIVERSAL_OBJ) $(ASM_XANIM_OBJ) \
	$(BOTLIB_OBJ) $(CLIENTSCR_OBJ) $(GAME_OBJ) $(QCOMMON_OBJ) $(SERVER_OBJ) $(SOUND_OBJ) $(STRINGED_OBJ) \
	$(UI_MP_OBJ) $(UNIVERSAL_OBJ) $(XANIM_OBJ) $(LINUX_OBJ) $(WIN32_OBJ) $(ZLIB_OBJ)
	$(CXX) $(LFLAGS) -o $@ $^ $(LLIBS)
...
# A rule to build sound source code.
$(OBJ_DIR)/%.o: $(SOUND_DIR)/asm/%.asm
	@echo $(NASM) $@
	@$(NASM) $(NASMFLAGS) $< -o $@

# A rule to build sound source code (test).
$(TEST_OBJ_DIR)/%.o: $(SOUND_DIR)/asm/%.asm
	@echo $(NASM) $@
	@$(NASM) $(NASMFLAGS) $< -o $@
...
# A rule to build ui_mp source code.
$(OBJ_DIR)/%.o: $(UI_MP_DIR)/asm/%.asm
	@echo $(NASM) $@
	@$(NASM) $(NASMFLAGS) $< -o $@

# A rule to build ui_mp source code (test).
$(TEST_OBJ_DIR)/%.o: $(UI_MP_DIR)/asm/%.asm
	@echo $(NASM) $@
	@$(NASM) $(NASMFLAGS) $< -o $@
