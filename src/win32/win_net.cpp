...
static dvar_t	*net_noudp;
static dvar_t	*net_noipx;

static dvar_t	*net_socksEnabled;
static dvar_t	*net_socksServer;
static dvar_t	*net_socksPort;
static dvar_t	*net_socksUsername;
static dvar_t	*net_socksPassword;
...
void NET_OpenIP( void ) {
	...
	ip = Dvar_RegisterString( "net_ip", "localhost", DVAR_LATCH );
	port = Dvar_RegisterInt( "net_port", PORT_SERVER, 0, UINT16_MAX, DVAR_LATCH );
	...
	for( i = 0 ; i < 10 ; i++ ) {
		...
		if ( ip_socket ) {
			port->current.integer = port->current.integer + i;
			port->latched.integer = port->latched.integer + i;
			if ( net_socksEnabled->current.boolean ) {
				NET_OpenSocks( port->current.integer );
			}
			NET_GetLocalAddress();
			return;
		}
...
void NET_OpenIPX( void ) {
	...
	port = Dvar_RegisterInt( "net_port", PORT_SERVER, 0, UINT16_MAX, DVAR_LATCH );
	ipx_socket = NET_IPXSocket( port->current.integer );
}
...
static qboolean NET_GetDvars( void ) {
	...
	net_noudp = Dvar_RegisterBool( "net_noudp", 0, DVAR_LATCH | DVAR_ARCHIVE );

	if( net_noipx && net_noipx->modified ) {
		modified = qtrue;
	}
	net_noipx = Dvar_RegisterBool( "net_noipx", 0, DVAR_LATCH | DVAR_ARCHIVE );


	if( net_socksEnabled && net_socksEnabled->modified ) {
		modified = qtrue;
	}
	net_socksEnabled = Dvar_RegisterBool( "net_socksEnabled", 0, DVAR_LATCH | DVAR_ARCHIVE );

	if( net_socksServer && net_socksServer->modified ) {
		modified = qtrue;
	}
	net_socksServer = Dvar_RegisterString( "net_socksServer", "", DVAR_LATCH | DVAR_ARCHIVE );

	if( net_socksPort && net_socksPort->modified ) {
		modified = qtrue;
	}
	net_socksPort = Dvar_RegisterInt( "net_socksPort", 1080, 0, UINT16_MAX, DVAR_LATCH | DVAR_ARCHIVE );

	if( net_socksUsername && net_socksUsername->modified ) {
		modified = qtrue;
	}
	net_socksUsername = Dvar_RegisterString( "net_socksUsername", "", DVAR_LATCH | DVAR_ARCHIVE );

	if( net_socksPassword && net_socksPassword->modified ) {
		modified = qtrue;
	}
	net_socksPassword = Dvar_RegisterString( "net_socksPassword", "", DVAR_LATCH | DVAR_ARCHIVE );
...
void NET_Config( qboolean enableNetworking ) {
	...
	if( net_noudp->current.boolean && net_noipx->current.boolean ) {
	...
	if( start ) {
		if (! net_noudp->current.boolean ) {
			NET_OpenIP();
		}
		if (! net_noipx->current.boolean ) {
			NET_OpenIPX();
		}
	}
}
