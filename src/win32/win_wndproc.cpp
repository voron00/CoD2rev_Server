...
dvar_t		*vid_xpos;			// X coordinate of window position
dvar_t		*vid_ypos;			// Y coordinate of window position
dvar_t		*r_fullscreen;
...
static void WIN_DisableAltTab( void )
{
	...
	if ( !Q_stricmp( Dvar_GetString( "arch" ), "winnt" ) )
	{
    ...
    LPARAM  lParam)
{
	...
	case WM_MOUSEWHEEL:
		...
		if (in_mouse->current.integer != 1 || (!r_fullscreen->current.integer && (cls.keyCatchers & KEYCATCH_CONSOLE)))
		{
			...
			if ( zDelta > 0 )
			{
				for(i=0; i<zDelta; i++)
				{
					if (!in_logitechbug->current.integer)
					{
						Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MWHEELUP, qtrue, 0, NULL );
						Sys_QueEvent( g_wv.sysMsgTime, SE_KEY, K_MWHEELUP, qfalse, 0, NULL );
					}
	...
	case WM_CREATE:
		...
		vid_xpos = Dvar_RegisterInt ("vid_xpos", 3, -4096, 4096, DVAR_ARCHIVE);
		vid_ypos = Dvar_RegisterInt ("vid_ypos", 22, -4096, 4096, DVAR_ARCHIVE);
	...
	case WM_MOVE:
		{
			...
			if (!r_fullscreen->current.integer )
			{
				...
				Dvar_SetInt( vid_xpos, xPos + r.left);
				Dvar_SetInt( vid_ypos, yPos + r.top);
...
// one update sometimes, so we always check all states and look for events
	...
	case WM_SYSKEYDOWN:
		if ( wParam == 13 )
		{
			...
			{
				Dvar_SetBool( r_fullscreen, !r_fullscreen->current.boolean );
				Cbuf_AddText( "vid_restart\n" );
			}
			return 0;
		}
