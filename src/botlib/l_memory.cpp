...
#endif //MEMDEBUG
{
	...
	ptr = Z_Malloc( size + sizeof( memoryblock_t ) );
...
#endif //MEMDEBUG
{
	...
	ptr = Hunk_Alloc( size + sizeof( memoryblock_t ) );
	block = (memoryblock_t *) ptr;
	block->id = HUNK_ID;
	block->ptr = (char *) ptr + sizeof( memoryblock_t );
	block->size = size + sizeof( memoryblock_t );
...
memoryblock_t *BlockFromPointer( void *ptr, char *str ) {
	...
	if ( !ptr ) {
		...
		//crash[0] = 1;
		Com_Printf( PRT_FATAL, "%s: NULL pointer\n", str );
#endif // MEMDEBUG
		return NULL;
	} //end if
	block = ( memoryblock_t * )( (char *) ptr - sizeof( memoryblock_t ) );
	if ( block->id != MEM_ID && block->id != HUNK_ID ) {
		Com_Printf( PRT_FATAL, "%s: invalid memory block\n", str );
		return NULL;
	} //end if
	if ( block->ptr != ptr ) {
		Com_Printf( PRT_FATAL, "%s: memory block pointer invalid\n", str );
		return NULL;
	} //end if
	return block;
...
void PrintUsedMemorySize( void ) {
	Com_Printf( PRT_MESSAGE, "total allocated memory: %d KB\n", allocatedmemory >> 10 );
	Com_Printf( PRT_MESSAGE, "total botlib memory: %d KB\n", totalmemorysize >> 10 );
	Com_Printf( PRT_MESSAGE, "total memory blocks: %d\n", numblocks );
...
#endif //MEMDEBUG
{
	...
	ptr = Z_Malloc( size + sizeof( unsigned long int ) );
	if ( !ptr ) {
		return NULL;
	}
...
#endif //MEMDEBUG
{
	...
	ptr = Hunk_Alloc( size + sizeof( unsigned long int ) );
	if ( !ptr ) {
		return NULL;
	}
	memid = (unsigned long int *) ptr;
