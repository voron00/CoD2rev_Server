...
int PC_EvaluateTokens( source_t *source, token_t *tokens, signed long int *intvalue,
	...
	while ( !error && firstoperator )
	{
...
#ifdef DEBUG_EVAL
		if ( integer ) {
			Log_Write( "operator %s, value1 = %d", PunctuationFromNum( source->scriptstack, o->operat ), v1->intvalue );
			if ( v2 ) {
		...
		{
			Log_Write( "operator %s, value1 = %f", PunctuationFromNum( source->scriptstack, o->operat ), v1->floatvalue );
			if ( v2 ) {
...
void PC_CheckOpenSourceHandles( void ) {
	...
	for ( i = 1; i < MAX_SOURCEFILES; i++ )
	{
		if ( sourceFiles[i] ) {
#ifdef BOTLIB
			Com_Printf( "file %s still open in precompiler\n", sourceFiles[i]->scriptstack->filename );
#endif  //BOTLIB
		} //end if
	} //end for
